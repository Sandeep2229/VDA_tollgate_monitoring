...
def displayVehicleCount(frame, vehicle_count,car_count,truck_count,bus_count):
	cv2.putText(
		frame, #Image
	...
	cv2.putText(
		frame, #Image
		'Detected cars: '+str(car_count), #Label
		(20, 55), #Position
		cv2.FONT_HERSHEY_SIMPLEX, #Font
		0.8, #Size
		(0 ,0xFF,0xFF), #Color
	...
	cv2.putText(
		frame, #Image
		'Detected truck: '+str(truck_count), #Label
		(20, 85), #Position
		cv2.FONT_HERSHEY_SIMPLEX, #Font
		0.8, #Size
		(0,0,0xFF), #Color
	...
	cv2.putText(
		frame, #Image
		'Detected bus: '+str(bus_count), #Label
		(20, 115), #Position
		cv2.FONT_HERSHEY_SIMPLEX, #Font
		0.8, #Size
		(0xFF,0,0), #Color
...
def count_vehicles(idxs, boxes, classIDs, vehicle_count,car_count,truck_count,bus_count, previous_frame_detections, frame):
	current_detections = {}
	# ensure at least one detection exists
	if len(idxs) > 0:
		# loop over the indices we are keeping
		for i in idxs.flatten():
			...
			if (LABELS[classIDs[i]] in list_of_vehicles):
				...
				if (not boxInPreviousFrames(previous_frame_detections, (centerX, centerY, w, h), current_detections)):
                                        coordinate_list = list(previous_frame_detections[9].keys())[0]
                                        print(FRAMES_BEFORE_CURRENT)
	...
	return vehicle_count,car_count,truck_count,bus_count, current_detections
...
num_frames, vehicle_count,car_count , truck_count, bus_count= 0, 0, 0, 0, 0
...
while True:
	...
	vehicle_count, car_count,truck_count,bus_count, current_detections = count_vehicles(idxs, boxes, classIDs, vehicle_count,car_count,truck_count,bus_count, previous_frame_detections, frame)

	# Display Vehicle Count if a vehicle has passed the line 
	displayVehicleCount(frame, vehicle_count,car_count,truck_count,bus_count)
